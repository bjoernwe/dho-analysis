...
def plot_slowness(
    ...
    for i in range(n_sfa_components):
        ...
        plt.plot(df_train.select(["date"]), df_train.select([f"SFA_{i}"]), color="navy", zorder=1)
    #for i in range(n_sfa_components):
    #    plt.figure()
    #    plt.title(f"SFA #{i}")
    #    plt.plot(df_train.select(["date"]), df_train.select([f"SFA_{i}"]), color="silver", zorder=1)
    #    plt.scatter(df_train.select(["date"]), df_train.select([f"SFA_{i}"]), c=df_train.select(["SFA_1"]), cmap="PiYG", s=40, alpha=.8, zorder=2)
